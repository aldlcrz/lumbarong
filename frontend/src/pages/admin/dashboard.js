...
export default function AdminDashboard() {
    ...
    const fetchPendingProducts = async () => {
        api.get('/products?status=pending')
            .then(res => setPendingProducts(res.data.filter(p => p.status === 'pending')))
            .catch(err => console.error(err));
    };
            ...
            <div className="max-w-[1600px] mx-auto flex flex-col lg:flex-row gap-8 p-6 lg:p-12">
                ...
                <main className="lg:flex-1 min-w-0">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
                        ...
                        <StatHUD title="Pending Products" value={data.pendingProductsCount || pendingProducts.length} icon={Store} badge="Review" color="text-red-500" />
                    ...
                    {/* Product Registry: Pending Listings */}
                    <section className="bg-white rounded-[3rem] p-10 border border-gray-100 shadow-sm mb-8">
                        <div className="flex items-center justify-between mb-8">
                            <div>
                                <h2 className="text-2xl font-black text-gray-900 tracking-tight italic">Product Registry</h2>
                                <p className="text-sm text-gray-400 font-medium mt-1">Pending approval for new heritage tokens.</p>
                            </div>
                        ...
                        {pendingProducts.length === 0 ? (
                            <div className="py-20 text-center border-2 border-dashed border-gray-50 rounded-[2rem]">
                                <Store size={48} className="mx-auto text-gray-100 mb-4" />
                                <p className="text-gray-300 font-bold italic">No pending listings found.</p>
                            </div>
                        ) : (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                {pendingProducts.map(product => (
                                    <div key={product.id} className="p-6 bg-gray-50/50 rounded-3xl border border-gray-100 hover:border-red-600 transition-all group overflow-hidden">
                                        <div className="flex gap-4 mb-6">
                                            <div className="w-20 h-20 rounded-2xl bg-white border border-gray-100 overflow-hidden shrink-0">
                                                <img src={product.images?.[0]?.url || product.images?.[0]} className="w-full h-full object-cover" />
                                            </div>
                                            <div className="min-w-0">
                                                <h3 className="font-black text-gray-900 uppercase tracking-tight truncate">{product.name}</h3>
                                                <p className="text-[10px] font-bold text-red-600 italic mb-2">₱{product.price?.toLocaleString() || '0'}</p>
                                                <div className="flex items-center gap-2">
                                                    <span className="text-[8px] bg-white px-2 py-0.5 rounded-full border border-gray-100 font-black uppercase text-gray-400">{product.category}</span>
                                                    <span className="text-[8px] bg-white px-2 py-0.5 rounded-full border border-gray-100 font-black uppercase text-gray-400">Stock: {product.stock}</span>
                                                </div>
                                            </div>
                                        </div>
                                        <div className="flex gap-2">
                                            <button
                                                ...
                                                disabled={loading}
                                                className="flex-1 px-4 py-2.5 rounded-xl bg-gray-900 text-white text-[10px] font-black uppercase tracking-widest hover:bg-red-600 transition-all shadow-lg shadow-gray-200 disabled:opacity-50"
                                            >
                                                Approve
                                            </button>
                                            <button
                                                ...
                                                disabled={loading}
                                                className="px-4 py-2.5 rounded-xl border border-gray-200 text-gray-400 text-[10px] font-black uppercase tracking-widest hover:border-red-600 hover:text-red-600 transition-all disabled:opacity-50"
                                            >
                                                Reject
                                            </button>
