...
export default function SellerLayout({ children }) {
    ...
    const menuItems = [
        ...
        { name: 'MY CATALOG', icon: Package, href: '/seller/inventory' },
        { name: 'ACTIVE ORDERS', icon: ShoppingBag, href: '/seller/orders' },
        { name: 'SHOP PROFILE', icon: Store, href: '/seller/profile' },
    ];
    ...
    const handleSearch = (e) => {
        if (e.key === 'Enter' && searchQuery.trim()) {
            router.push(`/seller/inventory?search=${searchQuery}`);
        }
    };
    ...
    return (
        <div className="min-h-screen bg-[#fdfbf7] flex font-sans selection:bg-red-600 selection:text-white">
            {/* Sidebar - Desktop */}
            <aside className="hidden lg:flex w-72 bg-white border-r border-gray-100 flex-col sticky top-0 h-screen shrink-0">
                ...
                <div className="px-4 flex-1">
                    <p className="px-4 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-4">Seller Workshop</p>
                ...
                <div className="p-6 mt-auto border-t border-gray-50">
                    <div className="flex items-center gap-3 bg-red-50 p-4 rounded-3xl border border-red-100/50 group relative">
                        <div className="w-12 h-12 rounded-2xl bg-red-600 flex items-center justify-center text-white font-black text-lg shadow-lg shadow-red-100 shrink-0 overflow-hidden">
                            {user.profileImage ? <img src={user.profileImage} className="w-full h-full object-cover" /> : user.shopName?.charAt(0) || user.name.charAt(0)}
                        </div>
                        <div className="min-w-0 flex-1">
                            <p className="text-sm font-black text-gray-900 truncate uppercase tracking-tighter">{user.shopName || 'Shop'}</p>
                ...
                <header className="h-20 bg-white border-b border-gray-100 flex items-center justify-between px-6 lg:px-12 sticky top-0 z-50">
                    ...
                    <div className="flex items-center gap-4">
                                ...
                                {isNotificationsOpen && (
                                    ...
                                    >
                                        <div className="px-4 py-3 border-b border-gray-50">
                                            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Workshop Alerts</p>
                                        </div>
                        ...
                        <div className="w-10 h-10 rounded-xl bg-red-600 flex items-center justify-center text-white font-black text-xs shadow-lg shadow-red-100 cursor-pointer hover:scale-105 transition-transform group overflow-hidden relative">
                            {user.profileImage ? <img src={user.profileImage} className="w-full h-full object-cover" /> : user.name.charAt(0)}
                        </div>
                    </div>
                </header>
